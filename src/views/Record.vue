<template>
  <div>
    ...
    <Loader v-if="loading" /> <!-- показывает процесс загрузки -->
...
export default {
  ...
  async mounted() {
    this.categories = await this.$store.dispatch('fetchCategories') // получаем список всех категорий
    this.loading = false
    ...
    setTimeout(() => {
      this.select = M.FormSelect.init(this.$refs.select) // инициализируем плагин select
      M.updateTextFields() // оживляем поле если есть значение
    }, 0)
  },
  computed: {
    ...mapGetters(['info']), // getter в info.js
    canCreateRecord() {
      if(this.type === 'income') {
        return true
      }
